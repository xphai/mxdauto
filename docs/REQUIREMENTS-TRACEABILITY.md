...
> **战略负责人**：5.6Sol Ultra  
> **战术包负责人**：5.6 Luna max  
> **当前阶段**：G-1 战略封存完成；G0=`PASS`；G1=`In Progress`（`G1-FRM-001A`=`Completed`，`G1-FRM-001B1`=`Completed`，`G1-FRM-001B2`=`In Progress / Technical Evidence PASS / Countersign Pending`）
...
| G1 | CAP-001、UI-001、CV-001/002、LOC-001、FUN-001 的 Replay/Shadow | **In Progress**：`G1-FRM-001A` 与 `G1-FRM-001B1` 已 Completed；001B2 technical evidence PASS，outer CI/组织会签待完成；完整 `G1-FRM-001`、OBS/LOC/WST/Planner/完整 Shadow 仍待实施 |
...
| G1-FRM-001 | 001A synthetic admission、001B1 software foundation 以及 001B2 hardware/corpus/Candidate technical evidence 已完成本地严格验证 | 001B2 protected packaging outer CI 与组织会签；随后执行完整 `G1-FRM-001` Gate 审计 | `In Progress`，真实输入 0 |
...
| G1-FRM-001B2 | source `37e57b9...` 的 300 秒 VC-003 smoke、4-session/300-sample corpus、3-run replay、4 Event Tapes、CAS/provenance/privacy/zero-input 与 G1 Frame Candidate packet | protected packaging outer CI、P/run 回填、QA/技术/现场/privacy/release/Sol-U 组织会签 | `In Progress / Technical Evidence PASS`，真实输入 0 |
...
- 边界：raw Pixel CAS/视觉 review sheet 保持私有；`signoffs=[]`，privacy report 的 organizational human countersign=`pending`；packaging commit `P` 与 outer main run 待 protected PR 合并后回填。
